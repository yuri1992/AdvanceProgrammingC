...
void mergeAscending(ListNode* nodeLst1, ListNode* nodeLst2, List *res) {
    if (nodeLst1 == NULL && nodeLst2 == NULL)
        ...
        insertDataToEndList(res, *(nodeLst2->dataPtr));
        mergeAscending(NULL, nodeLst2->next, res);
        return;
    }
    else if (nodeLst2 == NULL) {
        insertDataToEndList(res, *(nodeLst1->dataPtr));
        mergeAscending(nodeLst1->next, NULL, res);
        return;
    }
